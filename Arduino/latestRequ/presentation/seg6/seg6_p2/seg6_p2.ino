...
String phoneNumber="+94776669129"; //phone number to which notification messages are sent
...
void loop() { 
  ...
  if (req.indexOf("/vibration/0") != -1){
    ...
    digitalWrite(14, 0);
    s = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nsoilMoisture_Sensor_Down";}
    
  else if (req.indexOf("/soil/1") != -1){
    digitalWrite(14, 1);
    s = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nsoilMoisture_Sensor_Up";}
    
  else if (req.indexOf("/soil/check/1") != -1){
    s = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nSoil_moisture_is "+String((250-(int(analogRead(0))-750))/2.5)+"%";}
    ...
    s = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nPhotocell_sensor_deactivated";}

    
  else if (req.indexOf("/photocell/1") != -1){
    autoLightStatus=true;
    s = "HTTP/1.1 200 OK\r\nContent-Type: text/plain\r\n\r\nPhotocell_sensor_activated!";}
    ...
    if (!bAlarm){
      ...
      if (timeStatus() != timeNotSet) {
            if (now() != prevDisplay) { //update the display only if time has changed
                ...
                String url = "/sendsmspost.php?uname=happyhome.ucsc@gmail.com&pword=Happyhome.ucsc1994&message=Vibration%20detected%20in%20the%20living%20room.-HomeAssistent&selectednums="+phoneNumber+"&info=1&test=0";
